...
## ソースコードの説明
l.79:新たに挿入するデータの領域を確保する。

l.80:新なデータnewCityを代入する。

l.81:pNextにpNextを代入する。

l.83:返り値としてpNodeを返す。

l.109:pNodeにリストの先頭アドレスpListを代入する。

l.111:pNodeがNULLにならない間、検索を行う。

l.112~115:入力したIDとリスト中のIDが一致したら、pCityにデータを代入し、posを返す。
